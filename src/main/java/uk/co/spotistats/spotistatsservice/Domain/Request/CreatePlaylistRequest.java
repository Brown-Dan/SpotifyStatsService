...
public record CreatePlaylistRequest(SpotifyAuthData authData, String name, String description, List<String> trackUris) {
    ...
    public static CreatePlaylistRequest fromSpotifySearchRequest(SpotifySearchRequest spotifySearchRequest, List<StreamData> streamData) {
        return aCreatePlaylistRequest()
                .withName("Recent songs - %s".formatted(LocalDate.now()))
                .withDescription("Autogenerated playlist for my most recent %s songs - created at - %s".formatted(spotifySearchRequest.limit(), LocalDateTime.now()))
                .withTrackUris(streamData.stream().map(StreamData::trackUri).toList())
                .withSpotifyAuthData(spotifySearchRequest.authData())
                .build();
    }

    public static CreatePlaylistRequest fromTopStreamsSearchRequest(SpotifySearchRequest spotifySearchRequest, List<StreamData> streamData) {
        return aCreatePlaylistRequest()
                .withName("Top songs @ %s".formatted(LocalDate.now()))
                .withDescription("Autogenerated playlist for my top %s songs - created at - %s".formatted(spotifySearchRequest.limit(), LocalDateTime.now()))
                .withTrackUris(streamData.stream().map(StreamData::trackUri).toList())
                .withSpotifyAuthData(spotifySearchRequest.authData())
                .build();
    }
